...
$routes->get('register/tailor', 'AuthController::registerTailor', ['as' => 'register-tailor']);
$routes->post('register/tailor', 'AuthController::registerTailorDo');
...
// Measurements (customer routes)
$routes->get('measurements', 'MeasurementController::index', ['as' => 'measurements', 'filter' => 'login']);
$routes->get('measurements/(:num)', 'MeasurementController::index/$1', ['filter' => 'login']);
$routes->post('measurements/save', 'MeasurementController::save', ['filter' => 'login']);
...
$routes->group('orders', function($routes) {
    ...
    $routes->get('create/(:num)', 'OrderController::create/$1');
    $routes->post('store', 'OrderController::store');
    $routes->get('search_tailor', 'OrderController::searchTailor');
    $routes->get('tailor_details/(:num)', 'OrderController::tailorDetails/$1');
    $routes->get('view_bids/(:num)', 'OrderController::viewBids/$1');
    $routes->post('accept_bid', 'OrderController::acceptBid');

    // Tailor Routes
    $routes->get('bid_list', 'OrderController::bidList');
    $routes->get('customer_details/(:num)', 'OrderController::customerDetails/$1');
    $routes->get('customer_details2/(:num)', 'OrderController::customerDetails2/$1');
    $routes->post('submit_bid', 'OrderController::submitBid');
    $routes->post('update_status', 'OrderController::updateStatus');
    $routes->post('mark_pickup_done', 'OrderController::markPickupDone');
    $routes->get('my_orders', 'OrderController::myOrders');
    $routes->get('my_tailors_list', 'OrderController::myTailorsList');
    $routes->get('my_customer_list', 'OrderController::myCustomersList');
    $routes->get('my_customers_list', 'OrderController::myCustomersList');
