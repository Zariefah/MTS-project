<!--begin::navbar-->
<nav id="navbar" class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top<?= logged_in() ? ' expand' : '' ?>">
  <div class="container">
    <a href="<?= base_url() ?>" class="navbar-brand" style="padding: 2px 8px 0 8px;">
      <img src="<?= base_url(config('Config\AppInfo')->icon) ?>" alt="<?= esc(config('Config\AppInfo')->name) ?>" class="navbar-logo" style="height: 32px;" />
    </a>
    <button
    ...
    <div class="collapse navbar-collapse" id="navbarContent">
      ...
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
        ...
        <li class="nav-item me-2 mt-2">
          <span class="navbar-text text-light fw-bold"><?= config('Config\AppInfo')->name ?></span>
        </li>
      ...
      <ul class="navbar-nav">
        ...
        $userMenu = [
          'link' => [
            ...
            'label' => '<img src="' . $avatarUrl . '" class="sidebar-avatar" alt=""> ' .
              (logged_in()
                ? (in_roles('Customer') ? 'Customer' :
                   (in_roles('Tailor') ? 'Tailor' : lang('Navbar.user.not_logged_in')))
                : lang('Navbar.user.not_logged_in')
              )
          ],
          'dropdown' => [
            ...
            [ 'url' => base_url() . 'register', 'icon' => 'bi bi-person-fill-add', 'label' => "Register as Customer", 'permitted' => !logged_in() && !empty($settings) && $settings['allowRegistration'] ],
            [ 'url' => base_url() . 'register/tailor', 'icon' => 'bi bi-person-fill-add', 'label' => "Register as Tailor", 'permitted' => !logged_in() && !empty($settings) && $settings['allowRegistration'] ],
