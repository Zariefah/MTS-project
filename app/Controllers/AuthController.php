...
class AuthController extends BaseController {
  ...
  public function registerDo(): mixed {
    ...
    $user->ssm_registration = null;
    $this->authConfig->requireActivation === null ? $user->activate() : $user->generateActivateHash();

    //
    // Set role to 'Customer' (id=3) for customer registration
    //
    $users = $users->withRole('Customer');
   ...
   * --------------------------------------------------------------------------
   *
   * Displays the tailor registration page.
   *
   * @return mixed
   */
  public function registerTailor(): mixed {
    //
    // Redirect back if already logged in
    //
    if ($this->auth->check()) {
    ...
    return $this->_render($this->authConfig->views['register_tailor'], ['authConfig' => $this->authConfig]);
  }
   ...
   * --------------------------------------------------------------------------
   *
   * Attempt to register a new tailor.
   *
   * @return mixed
   */
  public function registerTailorDo(): mixed {
    //
    // Check if registration is allowed
    //
    if (!$this->settings->getSetting('allowRegistration')) {
    ...
    $allowedPostFields = array_merge(['password'], $this->authConfig->validFields, $this->authConfig->personalFields);
    $user = new User($this->request->getPost($allowedPostFields));
    $user->ssm_registration = $this->request->getPost('ssm_registration');
    $this->authConfig->requireActivation === null ? $user->activate() : $user->generateActivateHash();

    //
    // Set role to 'Tailor' (id=2) for tailor registration
    //
    $users = $users->withRole('Tailor');
