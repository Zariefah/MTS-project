...
if (!function_exists('user_profile_is_complete')) {
  /**
   * Whether the logged-in user has saved required profile fields (address, region).
   * Matches validation in UserController::profileEditDo.
   */
  function user_profile_is_complete(): bool {
    if (!logged_in()) {
      return true;
    }
    $uid = user_id();
    ...
    return strlen($address) >= 10 && $region !== '';
  }
}

if (!function_exists('customer_measurements_are_complete')) {
  /**
   * Whether the current customer has a measurement row with all fields filled (> 0).
   */
  function customer_measurements_are_complete(): bool {
    if (!logged_in() || !in_roles('Customer')) {
      return true;
    }
    $uid = user_id();
    if ($uid === null) {
    ...
    $model = model(\App\Models\MeasurementModel::class);
    $row = $model->where('user_id', $uid)->first();
    if (!$row) {
      return false;
    }
    $fields = [
      'leher', 'bahu', 'bahu_ke_dada', 'bahu_ke_pinggang', 'lilitan_dada', 'pinggang', 'punggung',
      'bahu_ke_lutut', 'pinggang_ke_lutut', 'labuh_kain', 'labuh_tangan', 'lilitan_kekek', 'lubang_tangan',
    ];
    foreach ($fields as $f) {
      $v = $row[$f] ?? null;
